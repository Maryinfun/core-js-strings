...
function sumOfCodes(str) {
  return str
    ? Array.from(str).reduce((sum, el) => sum + el.charCodeAt(0), 0)
    : 0;
}
...
function isPalindrome(str) {
  const reverse = str
    ...
    .join('')
    .toLowerCase();
  return str.replace(/[\s,!?]/g, '').toLowerCase() === reverse;
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    reverse += words[i].split('').reverse().join('').concat(' ');
  }
...
function extractNameFromTemplate(value) {
  return value.split(' ').slice(1).join(' ').split('').slice(0, -1).join('');
}
